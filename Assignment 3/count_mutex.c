...
long iter = 0, countAccurate = 0, countMutex = 0; 
int count = 0;
long* array;
...
void* Mutex (void* threadID)
{
    ...
    pthread_mutex_lock(&mutex);
        ...
        {
            for(long i = id*avgThreadsIter; i <= id*avgThreadsIter+avgThreadsIter - 1; i++)  //in order to iterate over the splitted parts of the array (e.g., from 0 to 49, from 50 to 100)
            {
                if(array[i] == 1)
                    countMutex+=1;
                iter+=1;
...
int main()
{
    ...
    for(int i = 0; i < 100; i++)
    {
        ...
        {
            pthread_create(&threadsArray[j], NULL, &Mutex,(void*) j); //create threads
        }

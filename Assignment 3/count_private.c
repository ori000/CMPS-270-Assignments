...
int main()
{
    srand(time(NULL));
    clock_t totalTime;
    long* numberOfOnes = (long*)malloc(sizeof(long)*length);
    long* totalTimeArray = (long*)malloc(sizeof(long)*length);
    long* numberOfIterations = (long*)malloc(sizeof(long)*length);
    ...
    for(int i = 0; i < 100; i++)
    {
        ...
        {
            pthread_create(&threadsArray[j], NULL, &private,(void*) j);
        }
